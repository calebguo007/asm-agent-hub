...
| 组别 | 策略 | 说明 |
|---|---|---|
| **Group A (ASM)** | TOPSIS 多准则决策 | 根据用户偏好权重，综合成本/质量/延迟/可用性评分，选择最优服务 |
| **Group B (Random)** | 随机选择 | 从同 taxonomy 候选服务中随机选取 |
| **Group C (Expensive)** | 最贵优先 | 始终选择单位成本最高的服务（模拟"贵=好"假设） |

### 测试场景

- 从 `manifests/` 加载真实 ASM manifest 数据（14 个服务，6 种 taxonomy）
...
- **scorer.py** — 直接导入 ASM 的 TOPSIS 评分逻辑
