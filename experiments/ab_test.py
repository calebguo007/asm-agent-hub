#!/usr/bin/env python3
"""ASM A/B 测试框架 — 对比 TOPSIS vs 随机 vs 最贵策略。

三组对照实验：
  - Group A (ASM):       使用 scorer.py 的 TOPSIS 算法选择最优服务
  - Group B (Random):    从候选服务中随机选择
  - Group C (Expensive): 始终选择价格最高的服务
...
class TaskRequest:
    ...
    preferences: Preferences         # 实际偏好权重
...
class SelectionRecord:
    ...
    quality_score: float
    uptime: float
    topsis_score: float              # 该服务在当前偏好下的 TOPSIS 得分
...
def generate_tasks(
    ...
    taxonomy 分布按实际 manifest 数量加权（有更多服务的类别出现更频繁）。
    """
    if rng is None:
        rng = random.Random()
...
def strategy_asm_topsis(
    ...
    """Group A: 使用 TOPSIS 选择最优服务。返回 (选中的服务, TOPSIS 得分)。"""
    if not services:
        return None, 0.0
...
def strategy_random(
    ...
    """Group B: 随机选择。同时计算该服务的 TOPSIS 得分作为参考。"""
    if not services:
        return None, 0.0
    chosen = rng.choice(services)
    # 计算该服务在 TOPSIS 框架下的得分
    results = score_topsis(services, preferences)
    topsis_score = 0.0
    for r in results:
...
def main():
    parser = argparse.ArgumentParser(
        description="ASM A/B 测试框架 — 对比 TOPSIS vs Random vs Expensive 策略",
...
示例:
  ...
  python ab_test.py --manifests ../manifests  # 指定 manifest 目录
    ...
    if not manifests:
        print(f"错误: 在 {args.manifests} 中未找到 .asm.json 文件。")
        sys.exit(1)
    ...
    print(f"   完成! 共 {len(records)} 条选择记录 ({elapsed:.2f}s)")
