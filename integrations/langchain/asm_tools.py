"""ASM LangChain Tools — 查询 ASM Registry 并对比服务。

提供两个 LangChain BaseTool 实现:
  - ASMRegistryTool:   自然语言查询 → filter + TOPSIS 评分 → top 3 结果
...
def _parse_natural_query(query: str) -> tuple[Constraints, Preferences]:
    """从自然语言查询中解析出 Constraints 和 Preferences。"""
...
class ASMRegistryTool(BaseTool):
    """查询 ASM Registry，返回符合条件的 top 3 服务列表。

    解析自然语言查询，加载本地 manifests，
    使用 scorer.py 的 filter + TOPSIS 逻辑评分排序。
    """
    ...
    def _run(self, query: str) -> str:
        """执行查询：解析 → 过滤 → TOPSIS 评分 → 返回 top K。"""
        manifests = self._load_manifests()
        if not manifests:
            return f"错误: 在 {self.manifests_dir} 中未找到 .asm.json 文件。"
        ...
        # 格式化输出
        lines = [
            f"ASM Registry 查询结果 (query: \"{query}\")",
            f"共 {len(results)} 个服务匹配，展示 Top {len(top_results)}:",
            f"评分方法: TOPSIS | 偏好权重: cost={preferences.cost}, "
...
class ASMComparisonTool(BaseTool):
    ...
    def _load_manifests_map(self) -> dict[str, dict]:
        """加载 manifests 并构建 service_id → manifest 映射。"""
        if self.manifests_dir not in self._manifests_cache:
            self._manifests_cache[self.manifests_dir] = load_manifests(self.manifests_dir)
