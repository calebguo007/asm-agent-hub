#!/usr/bin/env python3
"""ASM v0.3 Manifest 生成器 — 单元测试

使用 mock OpenAPI spec 测试 manifest 生成与校验功能。
"""
...
# Mock 3: 无 endpoint 信息，仅通过描述推断（TTS 服务）
MOCK_TTS_SPEC = {
    "openapi": "3.0.3",
    "info": {
...
# Mock 4: 带有 info 级别 x-pricing 的 embedding 服务
MOCK_EMBEDDING_SPEC = {
    "openapi": "3.1.0",
    "info": {
...
class TestValidation(unittest.TestCase):
    ...
    def test_validate_only_mode(self):
        """测试 --validate-only 模式（通过临时文件）"""
        schema = load_schema()
        if schema is None:
            self.skipTest("schema 文件不存在")

        # 生成一个合法 manifest 并写入临时文件
        manifest = generate_manifest(MOCK_LLM_CHAT_SPEC)
