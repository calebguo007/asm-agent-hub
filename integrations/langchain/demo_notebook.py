...
演示完整流程：创建 agent → 用 ASMRegistryTool 查询 → 选择服务 → ASMReceiptCallback 记录。
...
print(f"   ASMReceiptCallback: 已加载")
...
# %% Cell 3: 模拟 Agent 查询 — 寻找最佳 LLM 服务
# ============================================================

print("\n" + "=" * 70)
print("  场景 1: Agent 需要一个高质量的 LLM chat 服务")
print("=" * 70)
...
# 模拟 on_tool_end 回调 — 自动生成 receipt
print("\n--- Receipt 生成 ---")
receipt_callback.on_tool_end(output=result_1)


# %% Cell 4: 模拟 Agent 查询 — 寻找便宜的图像生成服务
# ============================================================

print("\n" + "=" * 70)
print("  场景 2: Agent 需要一个便宜的图像生成服务")
print("=" * 70)
...
# %% Cell 5: 模拟 Agent 查询 — 快速的 TTS 服务
# ============================================================

print("\n" + "=" * 70)
print("  场景 3: Agent 需要一个快速的 TTS 服务")
print("=" * 70)
...
print(f"\n共生成 {len(receipt_files)} 个 receipt 文件:\n")
...
for rf in receipt_files:
    ...
    print(f"   服务: {data['credentialSubject']['display_name']} "
          f"({data['credentialSubject']['service_id']})")
    print(f"   评分: {data['credentialSubject']['selection_score']}")
    print(f"   查询: {data['credentialSubject']['query']}")
...
print("\n" + "=" * 70)
print("  完整 LangChain Agent 集成示例（伪代码）")
print("=" * 70)
...
print("=" * 70)
print(f"""
  ✅ ASMRegistryTool  — 自然语言查询 → TOPSIS 评分 → Top 3 推荐
  ✅ ASMComparisonTool — 多服务 manifest 对比表
  ✅ ASMReceiptCallback — 自动记录选择理由 → IETF ACTA receipt

  生成了 {len(receipt_files)} 个 receipt 文件在: {RECEIPT_DIR}

  ASM 信任链:
  查询 → 评分选择 → Receipt 记录 → 信任积累 → 更好的未来选择
""")
