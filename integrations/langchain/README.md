...
将 ASM (Agent Service Manifest) 的服务发现与评分能力集成到 LangChain Agent 中。

## 组件

### ASMRegistryTool

LangChain `BaseTool`，支持自然语言查询 ASM Registry。

- 解析自然语言查询，自动推断 taxonomy 和偏好方向
- 使用 `scorer.py` 的 `filter_services` + `score_topsis` 进行评分排序
- 返回 Top 3 推荐服务及评分详情
...
LangChain `BaseCallbackHandler`，自动捕获服务选择并生成 IETF ACTA 格式的 receipt。
...
- `pydantic >= 2.0` — 数据验证
- ASM `scorer.py` — 评分逻辑（通过 sys.path 自动引入，无需额外安装）
