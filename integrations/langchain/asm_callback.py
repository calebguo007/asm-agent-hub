"""ASM Receipt Callback — 自动记录服务选择并生成 receipt payload。

当 LangChain Agent 通过 ASMRegistryTool 选择了某个服务后，
...
class ASMReceiptCallback(BaseCallbackHandler):
    """LangChain Callback Handler — 捕获 ASM 服务选择并生成 receipt。

    当 agent 调用 asm_registry 工具并获得结果后，自动:
    1. 从工具输出中提取被选中的 #1 服务信息
    ...
    def on_tool_end(
        ...
        **kwargs: Any,
        ...
        """工具执行完成后，解析结果并生成 receipt。"""
        # 只处理 ASM registry 工具的输出
        if not output or "ASM Registry 查询结果" not in output:
            return
    ...
    def _parse_selection(self, output: str) -> Optional[dict[str, Any]]:
        """从 ASMRegistryTool 的输出中解析 #1 服务信息。"""
    ...
    def _save_receipt(self, receipt: dict[str, Any]) -> None:
        ...
        if self.verbose:
            ...
            print(f"   服务: {receipt['credentialSubject']['display_name']} "
                  f"({receipt['credentialSubject']['service_id']})")
            print(f"   评分: {receipt['credentialSubject']['selection_score']}")
