#!/usr/bin/env python3
"""ASM A/B 测试分析脚本 — 读取 CSV 生成 Markdown 报告。
...
def generate_report(records: list[Record]) -> str:
    ...
    report.append("|---|---|---|")
    report.append("| **A_ASM** | TOPSIS 多准则决策 | 根据用户偏好权重，综合成本/质量/延迟/可用性评分 |")
    report.append("| **B_Random** | 随机选择 | 从同 taxonomy 候选服务中随机选取 |")
    report.append("| **C_Expensive** | 最贵优先 | 始终选择单位成本最高的服务 |")
    report.append("")
    ...
    report.append(f"3. **统计显著性**: A vs B p={p_ab:.6f} {'(显著)' if p_ab < 0.05 else '(不显著)'}，"
                  f"A vs C p={p_ac:.6f} {'(显著)' if p_ac < 0.05 else '(不显著)'}")
    report.append(f"4. **核心洞察**: ASM 的 TOPSIS 算法能在多维度偏好下做出更优的综合决策，"
                  f"尤其在用户偏好明确时（如 cost_first、quality_first）优势更为显著。")
    report.append("")
    report.append("---")
    report.append(f"*报告由 ASM A/B 测试框架自动生成 — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*")
...
def main():
    parser = argparse.ArgumentParser(
        description="ASM A/B 测试分析 — 从 CSV 生成 Markdown 报告",
    )
    parser.add_argument(
        "--csv", "-c",
    ...
    print(f"✅ 加载了 {len(records)} 条记录 from {args.csv}")
