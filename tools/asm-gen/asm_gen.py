...
def load_file(path: str) -> Dict[str, Any]:
    ...
    if path.endswith((".yaml", ".yml")):
        if yaml is None:
            print("错误: 需要 pyyaml 库来解析 YAML 文件。请运行: pip install pyyaml", file=sys.stderr)
            sys.exit(1)
    ...
    try:
        ...
        if yaml is not None:
            return yaml.safe_load(content)
        print("错误: 无法解析文件，请确保文件为 JSON 或 YAML 格式。", file=sys.stderr)
        sys.exit(1)


def load_schema() -> Optional[Dict[str, Any]]:
    """加载 ASM v0.3 schema 文件。"""
...
def cmd_generate(
    ...
    try:
        ...
        print(f"错误: 无法读取 OpenAPI spec {input_path}: {e}", file=sys.stderr)
        return 1
